...
//Local
        ...
        for(int i=0; i<k; i++){
            ...
            Pque.push({nearest.second, i, WaterPos[nearest.first]});//水源との最小距離、その家の番号、その水源の座標
        }
...
//Local
            ...
            //水源が変更されているので、水源と最も距離が近い家を求め直す
            // if(Pque.empty()) break;
            priority_queue<tuple<int, int, vec2>, vector<tuple<int, int, vec2>>, greater<tuple<int, int, vec2>>> tmpPque;
            while(!Pque.empty()){
                tuple<int, int, vec2> updateque = Pque.top();
                Pque.pop();
                pair<int, int> updatepair = NearestWater(HousePos[get<1>(updateque)]);
                tmpPque.push({updatepair.second, get<1>(updateque), WaterPos[updatepair.first]});
            }
            swap(Pque, tmpPque);
        }
        return;
    ...
    pair<int, int> NearestWater(vec2 house){
        ...
        return {nearest, mindis};//{最も近い水源、その距離}
    }
